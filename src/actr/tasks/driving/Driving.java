	...
	public void update (double time)
	{
		if (time <= endTime)
		{
			if ((Math.round(time * 100) / 10.00) % 3 == 0) {
				getModel().getAudio().addAural("signal", "signal", "Speech");
			}
...
//				getModel().getVision().moveVisual ("car", cc.x+5, cc.y+10);
	...
	public void eval (Iterator<String> it)
	{
		...
		else if (cmd.equals("reset-flag"))
		{
			mwFlag = simulation.env.time + 3;
		}
	}
	...
	public boolean evalCondition (Iterator<String> it)
	{
		...
		if (cmd.equals("is-over-lane") || cmd.equals("is-not-over-lane")){
			...
			boolean b = Math.abs(carPos.z) > 1.0 || simulation.env.time < mwFlag;
